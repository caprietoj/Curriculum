...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.informacionPersonal.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.informacion-personals.update", [$informacionPersonal->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="nombre">{{ trans('cruds.informacionPersonal.fields.nombre') }}</label>
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $informacionPersonal->nombre) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('profesion') ? 'is-invalid' : '' }}" type="text" name="profesion" id="profesion" value="{{ old('profesion', $informacionPersonal->profesion) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pagina_web') ? 'is-invalid' : '' }}" type="text" name="pagina_web" id="pagina_web" value="{{ old('pagina_web', $informacionPersonal->pagina_web) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $informacionPersonal->email) }}">
                @if($errors->has('email'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('telefono') ? 'is-invalid' : '' }}" type="text" name="telefono" id="telefono" value="{{ old('telefono', $informacionPersonal->telefono) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('idiomas') ? 'is-invalid' : '' }}" name="idiomas_id" id="idiomas_id">
                    @foreach($idiomas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('idiomas_id') ? old('idiomas_id') : $informacionPersonal->idiomas->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
